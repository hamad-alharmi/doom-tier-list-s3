// DOOM SMP — board API
// GET  /api/state        → public read of the live board
// POST /api/state        → save (requires x-admin-password header)
//   body {"verify":true} → password handshake only, nothing saved
...
async function dbSet(key, value) {
  // send in body (handles large JSON); fall back to URL-encoded path
  let r = await fetch(`${BASE()}/set/${encodeURIComponent(key)}`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${TOKEN()}` },
    body: value
  });
  if (!r.ok) {
    r = await fetch(`${BASE()}/set/${encodeURIComponent(key)}/${encodeURIComponent(value)}`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${TOKEN()}` }
    });
  }
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    const b = req.body;
    if (!b || !Array.isArray(b.players) || !b.bracket) {
    ...
    try {
      await dbSet('board', JSON.stringify(b));
    } catch (e) {
